...
| 用户名 | 密码 | 权限 | 备注 |
|--------|------|------|------|
| linzuxi | <REMOVED_SECRET> | vip | 内测用户 |
| danny | ***REDACTED*** | vip | 内测用户 |
| ivan | ***REDACTED*** | vip | 内测用户 |
| shengwei | <REMOVED_SECRET> | vip | 内测用户 |
| vanilla | ***REDACTED*** | vip | 内测用户 |
| nanjian | ***REDACTED*** | vip | 内测用户 |
| fengzhijie | ***REDACTED*** | vip | 内测用户 |
| lianghailin | ***REDACTED*** | vip | 内测用户 |
| demo | <REMOVED_SECRET> | free | 演示账号 |
| zhuanz | <REMOVED_SECRET> | admin | 管理员 |
| hfzq | <REMOVED_SECRET> | vip | 华福证券 |
...
3. 测试 API：
   ...
   curl -X POST https://touziagent.com/api/login \
     ...
     -d '{"username":"demo","password":"<REMOVED_SECRET>"}'
   ```
